...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        ...
        if (sPartyMenuInternal->actions[i] == MENU_MOVES || sPartyMenuInternal->actions[i] == MENU_EGG_MOVES
            || sPartyMenuInternal->actions[i] == MENU_TM_MOVES || sPartyMenuInternal->actions[i] == MENU_SUB_MOVES)
            fontColorsId = 6;
...
static bool8 DoesMonKnowFieldMove(void)
{
    u32 i,j, move;
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (!InBattlePike())
    {
        ...
        if ((((CheckBagHasItem(ITEM_HM_FLY, 1)) && (Overworld_MapTypeAllowsTeleportAndFly(gMapHeader.mapType) == TRUE))
        || ((FlagGet(FLAG_BADGE02_GET)) && (CheckBagHasItem(ITEM_HM_FLASH, 1)) && (gMapHeader.cave == TRUE) && !(FlagGet(FLAG_SYS_USE_FLASH)))
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUseEffect)
    {
        ...
        if (holdEffectParam == 0 || holdEffectParam == 10)
        {
        ...
        if (targetSpecies != SPECIES_NONE)
        {
            if (holdEffectParam == 0) // Rare Candy gets consumed, Candy Box doesn't
                RemoveBagItem(gSpecialVar_ItemId, 1);
            FreePartyPointers();
...
void ItemUseCB_PokeBall(u8 taskId, TaskFunc task)
{
    ...
    else
    {
        ...
        ScheduleBgCopyTilemapToVram(2);
        gTasks[taskId].func = task;
        if (currBall != ITEM_CHERISH_BALL || currBall != ITEM_SAFARI_BALL)
