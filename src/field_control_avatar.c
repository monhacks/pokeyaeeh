...
{
    if (FlagGet(FLAG_BADGE05_GET) == TRUE && CheckBagHasItem(ITEM_HM_SURF, 1) && IsPlayerFacingSurfableFishableWater() == TRUE)
        return EventScript_UseSurf;

    if (MetatileBehavior_IsWaterfall(metatileBehavior) == TRUE)
    {
        if (FlagGet(FLAG_BADGE08_GET) == TRUE && CheckBagHasItem(ITEM_HM_WATERFALL, 1) && IsPlayerSurfingNorth() == TRUE)
...
{
    if (FlagGet(OW_FLAG_NO_ENCOUNTER)) // Used as a flag as well as item
        return FALSE;
