...
// The roamer's status field is u8, but SetMonData expects status to be u32
    ...
    if (ROAMER(index)->levelScaling)
        ROAMER(index)->level = max(ChooseGiveMonLevel(), ROAMER(index)->level);
...
void HandleRoamerRespawnTimer(void)
{
    ...
    else if (ROAMER(gEncounteredRoamerIndex)->respawnMode == POKEDEX_RESPAWN)
    {
        // As long as the mon is not caught, it'll respawn the next day
        if (!GetSetPokedexFlag(gEncounteredRoamerIndex, FLAG_GET_CAUGHT))
            ROAMER(gEncounteredRoamerIndex)->daysToRespawn = 1;
    }
}
