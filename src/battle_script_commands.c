...
{
    MOVE_BIND, MOVE_WRAP, MOVE_FIRE_SPIN, MOVE_CLAMP, MOVE_WHIRLPOOL, MOVE_SAND_TOMB, MOVE_MAGMA_STORM, MOVE_INFESTATION, MOVE_SNAP_TRAP, MOVE_THUNDER_CAGE, MOVE_MAELSTROM,
};
...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        if (IsBattlerWeatherAffected(gBattlerTarget, B_WEATHER_SUN) && (move == MOVE_HEAT_WAVE))
        {
            // heat wave ignore acc checks in sun unless target is holding utility umbrella
            JumpIfMoveFailed(7, move);
            return TRUE;
        }
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
       ...
       || abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR)
    {
        critChance = -1;
             ...
             || gBattleMoves[move].effect == EFFECT_SNIPE_SHOT // Just for Snipe Shot
...
static void Cmd_getexp(void)
{
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[*expMonId], MON_DATA_HP) && GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) != GetLevelCap())
            {
...
static void Cmd_setsubstitute(void)
{
    ...
    {
        gBattleMoveDamage = GetNonDynamaxMaxHP(gBattlerAttacker) / factor; // one bit value will only work for pokemon which max hp can go to 1020(which is more than possible in games)
        if (gBattleMoveDamage == 0)
            gBattleMoveDamage = 1;
