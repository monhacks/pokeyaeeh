...
static const u8 sText_EVO_LEVEL_SHEDINJA[] = _("{LV}{UP_ARROW} to {STR_VAR_2}, party<6, 1x Poké Ball");
...
static const u8 sText_EVO_DARK_SCROLL[] = _("Scroll Of Darks is used");
static const u8 sText_EVO_WATER_SCROLL[] = _("Scroll Of Waters is used");
...
static void PrintAbilityDescScreenTextSmall(u8 windowId, const u8* str, u8 left, u8 top)
{
    u8 color[3];
    color[0] = TEXT_COLOR_TRANSPARENT;
    color[1] = TEXT_DYNAMIC_COLOR_6;
    ...
    AddTextPrinterParameterized4(windowId, FONT_NARROWER, left, top, 0, 0, color, 0, str);
}
...
static bool8 CalculateMoves(void)
{
    ...
    if ((species >= SPECIES_VENUSAUR_MEGA && species <= SPECIES_GROUDON_PRIMAL) || (species >= SPECIES_BUTTERFREE_MEGA && species <= SPECIES_KINGDRA_MEGA))
...
static void PrintStatsScreen_Abilities(u8 taskId)
{
    ...
    if (gTasks[taskId].data[5] == 0)
    {
        ...
        PrintAbilityDescScreenTextSmall(WIN_STATS_ABILITIES, gAbilityDescriptionPointers[ability0], abilities_x, abilities_y + 14);
        ...
        if (ability1 != ABILITY_NONE && ability1 != ability0)
        {
            ...
            PrintAbilityDescScreenTextSmall(WIN_STATS_ABILITIES, gAbilityDescriptionPointers[ability1], abilities_x, abilities_y + 44);
        }
    }
    else //Hidden abilities
    {
        ...
        PrintAbilityDescScreenTextSmall(WIN_STATS_ABILITIES, gAbilityDescriptionPointers[abilityHidden], abilities_x, abilities_y + 14);
    }
}
...
static u8 PrintEvolutionTargetSpeciesAndMethod(u8 taskId, u16 species, u8 depth, u8 base_i_offset, u32 alreadyPrintedIcons[], u32 *base_icon_i_offSet)
{
    ...
    for (i = 0; i < times; i++)
    {
        ...
        HandleTargetSpeciesPrintText(targetSpecies, base_x + depth_x*depth, base_y, base_y_offset, base_i);
