...
#define FRIENDSHIP_EVO_THRESHOLD 150
#else
...
//Hisuian Forms
    ...
    // Eternatus
    [SPECIES_ETERNATUS_PRIMAL - 1] = NATIONAL_DEX_ETERNATUS,
    // Urshifu
    ...
    [SPECIES_GARBODOR_MEGA - 1] = NATIONAL_DEX_GARBODOR,
    [SPECIES_MELMETAL_MEGA - 1] = NATIONAL_DEX_MELMETAL,
    [SPECIES_RILLABOOM_MEGA - 1] = NATIONAL_DEX_RILLABOOM,
    [SPECIES_CINDERACE_MEGA - 1] = NATIONAL_DEX_CINDERACE,
    [SPECIES_INTELEON_MEGA - 1] = NATIONAL_DEX_INTELEON,
    [SPECIES_CORVIKNIGHT_MEGA - 1] = NATIONAL_DEX_CORVIKNIGHT,
    [SPECIES_ORBEETLE_MEGA - 1] = NATIONAL_DEX_ORBEETLE,
    [SPECIES_DREDNAW_MEGA - 1] = NATIONAL_DEX_DREDNAW,
    [SPECIES_COALOSSAL_MEGA - 1] = NATIONAL_DEX_COALOSSAL,
    [SPECIES_SANDACONDA_MEGA - 1] = NATIONAL_DEX_SANDACONDA,
    [SPECIES_CENTISKORCH_MEGA - 1] = NATIONAL_DEX_CENTISKORCH,
    [SPECIES_HATTERENE_MEGA - 1] = NATIONAL_DEX_HATTERENE,
    [SPECIES_GRIMMSNARL_MEGA - 1] = NATIONAL_DEX_GRIMMSNARL,
    [SPECIES_COPPERAJAH_MEGA - 1] = NATIONAL_DEX_COPPERAJAH,
    [SPECIES_URSHIFU_SINGLE_STRIKE_STYLE_MEGA - 1] = NATIONAL_DEX_URSHIFU,
    [SPECIES_URSHIFU_RAPID_STRIKE_STYLE_MEGA - 1] = NATIONAL_DEX_URSHIFU,
#endif
#if P_GEN_9_POKEMON == TRUE
    // Oinkologne
...
{   // Gen 1
    ...
    [SPECIES_TERAPAGOS_NORMAL - 1]             = ANIM_V_SQUISH_AND_BOUNCE,
    [SPECIES_TERAPAGOS_TERASTAL - 1]           = ANIM_V_SQUISH_AND_BOUNCE,
    ...
    [SPECIES_ETERNATUS_PRIMAL - 1]          = ANIM_V_SQUISH_AND_BOUNCE,
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI)
{
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
            // Rare Candy / EXP Candy / Candy Box
             ...
             && GetMonData(mon, MON_DATA_LEVEL, NULL) != MAX_LEVEL)
            {
                ...
                {
                    dataUnsigned = gExperienceTables[gSpeciesInfo[GetMonData(mon, MON_DATA_SPECIES, NULL)].growthRate][GetLevelCap()];
                }
    ...
    case EVO_MODE_NORMAL:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_NIGHT_SPECIFIC_MAP:
                currentMap = ((gSaveBlock1Ptr->location.mapGroup) << 8 | gSaveBlock1Ptr->location.mapNum);
                if (GetTimeOfDay() == TIME_NIGHT && gEvolutionTable[species][i].param <= level && gEvolutionTable[species][i].param2 == currentMap)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            }
        }
    ...
    case EVO_MODE_CANT_STOP:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_LEVEL_ITEM_COUNT_999:
                if (CheckBagHasItem(gEvolutionTable[species][i].param, 999))
                {
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                    RemoveBagItem(gEvolutionTable[species][i].param, 999);
                }
                break;
            }
        }
...
static void SortMovesAlphabetically(u16 *moves, u8 numMoves)
{
    ...
    for (i = 0; i < numMoves - 1; i++)
    {
        for (j = 0; j < numMoves - i - 1; j++)
        {
            ...
            compareResult = strcmp(gMoveNames[moveA], gMoveNames[moveB]);
...
const u8 *GetTrainerPartnerName(void)
{
    ...
    {
        if (gPartnerTrainerId == TRAINER_PARTNER(PARTNER_WALLY))
        {
            return gTrainers[TRAINER_WALLY_MAUVILLE].trainerName;
        }
        else
        {
...
u8 GetLevelCap(void)
{
    ...
    if (FlagGet(FLAG_SYS_GAME_CLEAR)) // Postgame: 100
        ...
        currentLevelCap = 65;
    else if (FlagGet(FLAG_BADGE05_GET)) // Gym 6: 55
        currentLevelCap = 55;
        ...
        currentLevelCap = 10; // Starting level cap
