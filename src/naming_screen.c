...
static bool8 KeyboardKeyHandler_Character(u8 input)
{
    ...
    if (input == INPUT_A_BUTTON)
    {
        ...
        // Naming screen will swap from "UPPER" to "lower" tab unless its for mystery gift
        if (sNamingScreen->templateNum != NAMING_SCREEN_MYSTERY_GIFT)
        {
...
{
    .copyExistingString = FALSE,
    .maxChars = MYSTERGY_GIFT_CODE_LENGTH,
    .iconFunction = 5,
    .addGenderIcon = FALSE,
    .initialPage = KBPAGE_LETTERS_UPPER,
    .unused = 35,
    .title = gText_MysteryGiftCode,
};
