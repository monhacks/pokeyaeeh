...
#define IMPENETRABLE_CHECK \
if (ability == ABILITY_IMPENETRABLE) \
{\
    RecordAbilityBattle(battler, ability);\
    gBattleStruct->turnEffectsTracker++;\
            break;\
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_REBOUND:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && TARGET_TURN_DAMAGED
             && GetBattleMoveSplit(move) == SPLIT_SPECIAL)
            {
                ...
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_ReboundActivates;
                effect++;
            }
            break;
...
{
    return TRUE; // Mew and Deoxys will no longer not-obey you :)
}
...
static bool32 IsBattlerGrounded2(u32 battler, bool32 considerInverse)
{
    ...
    if (GetBattlerAbility(battler) == ABILITY_LEVITATE)
        return FALSE;
    if (GetBattlerAbility(battler) == ABILITY_DRAGONFLY)
        return FALSE;
...
u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    {
    case EFFECT_FACADE:
        if ((gBattleMons[battlerAtk].status1 & (STATUS1_BURN | STATUS1_PSN_ANY | STATUS1_PARALYSIS | STATUS1_FROSTBITE | STATUS1_SLEEP)) || (atkAbility == ABILITY_COMATOSE))
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    case HOLD_EFFECT_LIGHT_BALL:
        if ((atkBaseSpeciesId == SPECIES_PIKACHU || atkBaseSpeciesId == SPECIES_PICHU) && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IS_MOVE_SPECIAL(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        if ((atkBaseSpeciesId == SPECIES_RAICHU) && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IS_MOVE_SPECIAL(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
...
static uq4_12_t GetWeatherDamageModifier(u32 battlerAtk, u32 move, u32 moveType, u32 holdEffectAtk, u32 holdEffectDef, u32 weather)
{
    if (weather == B_WEATHER_NONE)
        return UQ_4_12(1.0);
    if ((gBattleMoves[move].effect == EFFECT_HYDRO_STEAM || gBattleMoves[move].effect == EFFECT_STEAM_ERUPTION) && (weather & B_WEATHER_SUN) && holdEffectAtk != HOLD_EFFECT_UTILITY_UMBRELLA)
        return UQ_4_12(1.5);
                            ...
                            u32 holdEffectAtk, u32 holdEffectDef, u32 abilityAtk, u32 abilityDef)
{
    ...
    DAMAGE_APPLY_MODIFIER(GetBoxerBarrageModifier(battlerAtk));
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    else if (moveType == TYPE_GROUND && !IsBattlerGrounded2(battlerDef, TRUE) && !(gBattleMoves[move].ignoreTypeIfFlyingAndUngrounded))
    {
        ...
        else if (recordAbilities && defAbility == ABILITY_DRAGONFLY)
        {
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (moveType == TYPE_GROUND && (abilityDef == ABILITY_LEVITATE || abilityDef == ABILITY_DRAGONFLY) && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
...
bool32 CanBattlerGetOrLoseItem(u32 battler, u16 itemId)
{
    ...
    if (itemId == ITEM_ENIGMA_BERRY_E_READER)
        ...
        return FALSE;
         ...
         && ((gSpeciesInfo[gBattleMons[gBattlerAttacker].species].flags == SPECIES_FLAG_PARADOX) || (gSpeciesInfo[gBattleMons[gBattlerTarget].species].flags == SPECIES_FLAG_PARADOX)))
        return FALSE;
    else
        return TRUE;
}
...
bool8 CanMonParticipateInSkyBattle(struct Pokemon *mon)
{
    ...
    bool8 hasDragonflyAbility = gSpeciesInfo[species].abilities[monAbilityNum] == ABILITY_DRAGONFLY;
    ...
    {
        if ((hasLevitateAbility || hasDragonflyAbility || isFlyingType) && !IsMonBannedFromSkyBattles(species))
            return TRUE;
    }
    return FALSE;
}
