...

{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (aiMove != MOVE_NONE)
        {
            ...
            || aiMoveEffect == EFFECT_SLEEP || aiMoveEffect == EFFECT_YAWN || aiMoveEffect == EFFECT_TOXIC || aiMoveEffect == EFFECT_WILL_O_WISP || aiMoveEffect == EFFECT_PARALYZE || aiMoveEffect == EFFECT_FROST_GLARE
...
static bool32 IsMonGrounded(u16 heldItemEffect, u32 ability, u8 type1, u8 type2)
{
    ...
    if (type1 == TYPE_FLYING || type2 == TYPE_FLYING || ability == ABILITY_LEVITATE || ability == ABILITY_DRAGONFLY
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        if (item != ITEM_SAFETY_GOGGLES)
        {
            ...
            && ability != ABILITY_OVERCOAT && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
            {
                ...
                && ability != ABILITY_OVERCOAT && ability != ABILITY_SAND_VEIL && ability != ABILITY_SAND_RUSH && ability != ABILITY_SAND_FORCE && ability != ABILITY_DESERT_GALE))
            {
                weatherImpact = maxHP / 16;
                if (weatherImpact == 0)
                    weatherImpact = 1;
