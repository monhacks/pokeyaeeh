...
u32 AI_WhichMoveBetter(u32 move1, u32 move2, u32 battlerAtk, u32 battlerDef, s32 noOfHitsToKo)
{
    ...
    {
        if (gBattleMoves[move1].makesContact && !gBattleMoves[move2].makesContact)
            return -1;
        if (gBattleMoves[move2].makesContact && !gBattleMoves[move1].makesContact)
            return 1;
    }
...
bool32 AI_IsBattlerGrounded(u32 battlerId)
{
    ...
    if (holdEffect == HOLD_EFFECT_IRON_BALL)
        ...
        return FALSE;
    else if (AI_DATA->abilities[battlerId] == ABILITY_DRAGONFLY)
        return FALSE;
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if ((weather & B_WEATHER_RAIN || B_WEATHER_SANDSTORM)
      && (move == MOVE_HEAT_WAVE))
        return FALSE;
    ...
    if ((((weather & B_WEATHER_RAIN) && (gBattleMoves[move].effect == EFFECT_THUNDER || gBattleMoves[move].effect == EFFECT_HURRICANE))
            || (((weather & (B_WEATHER_HAIL | B_WEATHER_SNOW)) && (move == MOVE_BLIZZARD || gBattleMoves[move].effect == EFFECT_HURRICANE)))
            || ((weather & B_WEATHER_SUN) && (move == MOVE_HEAT_WAVE)))
...
bool32 ShouldSetSun(u32 battlerAtk, u32 atkAbility, u32 holdEffect)
{
    ...
    if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA
     && (atkAbility == ABILITY_CHLOROPHYLL
      ...
      || HasMoveEffect(battlerAtk, EFFECT_HYDRO_STEAM)
      || HasMoveEffect(battlerAtk, EFFECT_STEAM_ERUPTION)
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    if (flags & SIDE_STATUS_SPIKES && ((type1 != TYPE_FLYING && type2 != TYPE_FLYING
        && (ability != ABILITY_LEVITATE || ability != ABILITY_DRAGONFLY) && holdEffect != HOLD_EFFECT_AIR_BALLOON)
...
bool32 PartnerMoveEffectIsTerrain(u32 battlerAtkPartner, u32 partnerMove)
{
    ...
    if (partnerMove != MOVE_NONE
     && (gBattleMoves[partnerMove].effect == EFFECT_GRASSY_TERRAIN
      ...
      || gBattleMoves[partnerMove].effect == EFFECT_METAL_TERRAIN))
        return TRUE;
